...
/**
 * Fill statistic slots from videx-statistic-server response
 */
static void
...
/**
 * Hook of get_relation_stats, pg fetch statistic of base table without index from
 * this function
 */
static bool videx_get_relation_stats(PlannerInfo *root,
                                              RangeTblEntry *rte,
    ...
    if (rte->rtekind == RTE_RELATION){
        ...
        if (error) {
            elog(WARNING, "videx_get_relation_stats ask_from_videx_http error.");
            set_vardata_stats_from_syscache(vardata, rte->relid, attnum, rte->inh);
        } else {
...
/**
 * Hook of get_relation_stats, pg fetch statistic of base table with index from
 * this function
 */
static bool videx_get_index_stats(PlannerInfo *root,
                                           Oid indexOid,
