...
int ha_videx::info_low(uint flag, bool is_analyze)
{
	...
	int error = ask_from_videx_http(request_item, res_json, thd);
  	if (error) {
  		...
  		res_json["stat_n_rows"] = "25";
		res_json["data_file_length"] = "16384";
		res_json["index_file_length"] = "16384";
  		res_json["data_free_length"] = "0";
  	}
	else {
		...
		for (uint i = 0; i < table->s->keys; i++) {
			...
			for (j = 0; j < key->ext_key_parts; j++) {
				...
				*/
			    ...
			    std::string concat_key = std::string("rec_per_key #@# ") + key->name.str + " #@# " + key->key_part[j].field->field_name.str;
